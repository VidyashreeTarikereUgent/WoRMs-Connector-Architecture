...
function convertRMLtoYAML(rml, prefixes = {}) {
  ...
  // First, create labels for all TMs
  triplesMaps.forEach(tm => {
    ...
    if (temps.length === 0) {
      ...
      uriToYAMLName[tm.value] = mappingName
    }
  });
  triplesMaps.forEach(tm => {
    const temps = store.getQuads(null, namedNode(namespaces.fnml + 'functionValue'), tm).map(a => a.subject);
    ...
    if (temps.length === 0) {
      let mappingName = uriToYAMLName[tm.value];
      ...
      predicateObjectMaps.forEach(pom => {
        let canUseArrayShortcut = true; // if a function or join is used, I cannot use the `- [p, o]` shortcut
        let predicates = [];
        let objects = [];
        ...
        objectMaps.forEach(om => {
          ...
          const parentTriplesMaps = store.getQuads(om, namedNode(namespaces.rr + 'parentTriplesMap'), null).map(a => a.object);

          if(parentTriplesMaps.length > 0) {
            canUseArrayShortcut = false;
            object = {
              mapping: uriToYAMLName[parentTriplesMaps[0].value]
            }

            const joinConditions = store.getQuads(om, namedNode(namespaces.rr + 'joinCondition'), null).map(a => a.object);
            if(joinConditions.length > 0) {
              if(store.getQuads(joinConditions[0], namedNode(namespaces.rdf + 'type'), namedNode(namespaces.fnml + 'FunctionTermMap').length > 0)) {
                throw new Error('Join conditions via functions currently unsupported!')
              } else {
                object.condition = {
                  function: 'equal',
                  parameters: [
                    ['str1', store.getQuads(joinConditions[0], namedNode(namespaces.rr + 'child'), null).map(a => a.object).value, 's'],
                    ['str2', store.getQuads(joinConditions[0], namedNode(namespaces.rr + 'parent'), null).map(a => a.object).value, 'o'],
          ...
          else if (functionValues.length > 0) {
            canUseArrayShortcut = false;
