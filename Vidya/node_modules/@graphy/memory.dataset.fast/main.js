...
class Normalizer {
	constructor(kd_source) {
		...
		this._hc4_quads = kd_source._h_quad_tree;
		this._k_issuer = new Issuer('_:r');
		this._h_blanks = null;
		this._h_hashes = null;
	}

	// implementation of §4.4.2: Normalization Algorithm
	normalize() {
		const {
			_k_issuer: k_issuer,
		...
		const hc4_out = {[$_KEYS]:0, [$_QUADS]:0};
		let c_quads_quads = 0;
		let c_graphs = 0;
		...
		for(let sc1_graph of hc4_quads) {
			...
			for(let sc1_subject of hc3_trips) {
				...
				// open subject
				const hc2_out = hc3_out[sc1_subject] = {[$_KEYS]:0, [$_QUADS]:0};
				let c_quads_objects = 0;
				let c_predicates = 0;
				...
				for(const sc1_predicate of hc2_probs) {
					...
					// open predicate
					const as_out = hc2_out[sc1_predicate] = new Set();
					...
					for(let sc1_object of as_objects) {
						...
						as_out.add(sc1_object);
						c_quads_objects += 1;
					}
				}

				c_quads_trips += c_quads_objects;

				hc2_out[$_QUADS] = c_quads_objects;
				hc2_out[$_KEYS] = c_predicates;
			}

			c_quads_quads += c_quads_trips;

			hc3_out[$_QUADS] = c_quads_trips;
			hc3_out[$_KEYS] = c_subjects;
		}

		hc4_out[$_QUADS] = c_quads_quads;
		hc4_out[$_KEYS] = c_graphs;

		// (8) deliver canonicalized dataset
		return new FastDataset({
			prefixes: Object.assign({}, this._kd_source._h_prefixes),
		}, hc4_out);
	}

	// implementation of §4.6.2: Hash First Degree Quads Algorithm
	hash_1dq(sc1_blank) {
		const {
			_h_blanks: h_blanks,
			_hc4_quads: hc4_quads,
		...
		if(g_blank.graph) {
			...
			for(const sc1_subject of hc3_trips) {
				...
				for(const sc1_predicate of hc2_probs) {
					...
					// create norm string
					const s_norm_gsp = s_norm_gs+sc1_predicate+'\r';
		...
		for(const sc1_graph of g_blank.subject) {
			...
			// create norm string
			const s_norm_gs = ('_' === sc1_graph[0]? '_:z\n': sc1_graph+'\n')+'_:a\t';
		...
		for(const [sc1_graph, sc1_subject, sc1_predicate] of g_blank.object) {
			// init norm string
			let si_norm_gs = sc1_graph+'\n';

			// graph is blank node
			if('_' === sc1_graph[0]) {
				si_norm_gs = sc1_graph === sc1_blank? '_:a\n': '_:z\n';
			}

			// subject is blank node
			if('_' === sc1_subject[0]) {
				si_norm_gs += sc1_subject === sc1_blank? '_:a\t': '_:z\t';
			}
			else {
				si_norm_gs += sc1_subject+'\t';
			}

			// push norm string
			a_norms.push(si_norm_gs+sc1_predicate+'\r_:a');
		}
	...
	// implementation of §4.7.2: Hash Related Blank Node Algorithm
	...
	// helper method for (3) in §4.8.2: Hash N-Degree Quads
...
// 	let h_quads_b = k_other._h_quad_tree;
	...
	// 	// compare digests
	// 	return (new Normalizer(this)).normalize().string === (new Normalizer(k_other)).normalize().string;
// }
			...
			if(p_graph in h_quads_b) {
				...
				for(let p_subject in h_triples_a) {
					...
					if(p_subject in h_triples_b) {
						...
						for(let p_predicate in h_pairs_a) {
							// that is not in b
							if(p_predicate in h_pairs_b) continue;

							// clone object set
							const as_objects_r = new Set(h_pairs_a[p_predicate]);
...
Object.assign(FastDataset.prototype, {
	...
	canonicalize: FastDataset.prototype.normalize,
});
